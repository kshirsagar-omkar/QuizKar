...
-- ===============================
-- 3. QUIZ TABLE (Added time_taken)
-- ===============================
CREATE TABLE quiz (
    quiz_id SERIAL PRIMARY KEY,
    title VARCHAR(100) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    time_taken INT NOT NULL, -- New Column
    created_by INT NOT NULL,
    FOREIGN KEY (created_by) REFERENCES users(user_id) 
        ON UPDATE CASCADE ON DELETE CASCADE
);
...
-- ============================
-- 4. INSERT QUIZZES (Added time_taken)
-- ============================
INSERT INTO quiz (title, created_by, time_taken) VALUES 
...
-- 3. Get ALL Quizzes (Updated to show time_taken)
SELECT quiz_id, title, time_taken, created_at 
FROM quiz;

-- 4. Get Quizzes Given by Specific User (User2)
SELECT q.quiz_id, q.title, q.time_taken, lb.score, lb.participation_date
FROM quiz q
JOIN leaderboard lb ON q.quiz_id = lb.quiz_id
WHERE lb.user_id = 3;

-- 5. Global Leaderboard (Ordered by score, date, time_taken)
SELECT u.username, q.title, lb.score, lb.participation_date, q.time_taken
FROM leaderboard lb
JOIN users u ON lb.user_id = u.user_id
JOIN quiz q ON lb.quiz_id = q.quiz_id
ORDER BY lb.score DESC, lb.participation_date ASC, q.time_taken ASC;
