...
public class EmailServiceImpl implements EmailService {
	...
	private static final ExecutorService emailExecutor = Executors.newFixedThreadPool(5); // 5 concurrent threads 
	...
	//Send the mail,through emailExecutor (thread); without blocking the main thread
	public Future<Boolean> sendOTPViaMailAsync(OTPVerification otpVerification) {
		return emailExecutor.submit(() -> {
            try {
                return sendOTPViaMail(otpVerification);
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }
	...
	//When application, will be closed or re-deployed, at the time, shut down all executors (threads), to prevent memory leaks
	...
	public Boolean sendOTPViaMail(OTPVerification otpVerification) throws SQLException, MessagingException {
		...
		// String body = "Your OTP is: " + otp + "\nNote: It is valid for the next 5
		// minutes.";
	...
	public static void main(String[] args) throws SQLException, MessagingException {
		...
		otpVerification.setUserEmail("dnyaneshwarpadule83@gmail.com");
	...
	private String getBodyForOTP(String otp) {
		return "<!DOCTYPE html>" + "<html>" + "<head>" + "    <style>"
				...
				+ "        .divider { height: 1px; background: #eee; margin: 25px 0; }" + "    </style>" + "</head>"
				+ "<body>" + "    <div class='container'>" + "        <div class='header'>"
				+ "            <div class='logo'>QuizKar</div>" + "        </div>" + "        <div class='content'>"
				...
				+ "            © 2025 QuizKar. All rights reserved.<br>" + "            <div style='margin-top: 8px;'>"
